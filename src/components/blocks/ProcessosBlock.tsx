        ...
        {localData.processos.map((processo, index) => (
          <Card key={index}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <Input
                    placeholder="Nome do processo (ex: Onboarding de Cliente)"
                    value={processo.nome}
                    onChange={(e) => updateProcesso(index, 'nome', e.target.value)}
            ...
            <CardContent className="space-y-4">
              <Textarea
                placeholder="Descreva as etapas principais deste processo..."
                value={processo.descricao}
                onChange={(e) => updateProcesso(index, 'descricao', e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateProcesso(index, 'responsavel', e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-sm text-muted-foreground mb-1 block">Frequência</label>
                  <Select
                    value={processo.frequencia}
                    onValueChange={(value) => updateProcesso(index, 'frequencia', value)}
                  >
                    <SelectTrigger>
      ...
      {localData.processos.length === 0 && (
        <Card className="border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Clique em "Adicionar Processo" para mapear os processos essenciais
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="bg-muted/50">
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-primary">{localData.processos.length}</p>
                <p className="text-xs text-muted-foreground">Processos mapeados</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-primary">
                  {localData.processos.filter(p => p.responsavel).length}
                </p>
                <p className="text-xs text-muted-foreground">Com responsável</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-primary">
                  {localData.processos.filter(p => p.frequencia).length}
                </p>
                <p className="text-xs text-muted-foreground">Com frequência</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-primary">
                  {localData.processos.filter(p => p.descricao).length}
                </p>
